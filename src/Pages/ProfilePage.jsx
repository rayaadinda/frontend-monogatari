...
const ProfilePage = () => {
	...
	const fetchProfile = async (id) => {
		try {
			const response = await axios.get(
				`${import.meta.env.VITE_SERVER_DOMAIN}/api/users  /${id}`
			)
	...
	const handleFileChange = async (e) => {
		...
		try {
			const urlResponse = await axios.get(
				`${import.meta.env.VITE_SERVER_DOMAIN}/image-upload-url`,
				{
					headers: {
	...
	const handleHeaderPhotoChange = async (e) => {
		...
		try {
			...
			toast.error(
				err.response?.data?.error || "Failed to update header photo",
				{
					id: loadingToast,
				}
			)
	...
	return (
		<div className="min-h-screen bg-white">
			<div className="w-full">
				{/* Header Photo */}
				<div
					className="w-full h-48 bg-gradient-to-r from-gray-800 to-gray-900"
					style={{
						backgroundImage: `url(${
							headerPhotoPreviewUrl || profile?.personal_info?.header_img
						})`,
								...
								<div className="flex items-center space-x-4 text-base">
									<span>
										<span className="font-semibold">
											{profile?.account_info?.followers?.length || 0}
										</span>{" "}
										Followers
									</span>
									<span>
										<span className="font-semibold">
											{profile?.account_info?.following?.length || 0}
										</span>{" "}
										Following
									</span>
									<span>
										Joined{" "}
										{new Date(
											profile?.personal_info?.createdAt
